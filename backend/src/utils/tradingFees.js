const { calculateProjectedPnl, calculateQuantityFromLots, calculateUsdFromLots, getInstrumentTradingMeta } = require('./calculators');
...
const calculatePositionFees = ({
    ...
    const resolvedLots = parsedLots > 0
        ? parsedLots
        : (() => {
            const meta = getInstrumentTradingMeta({ symbol, category, instrument });
...
const calculatePositionNetState = ({
    symbol = '',
    category = '',
    instrument = {},
    side = 'buy',
    ...
    const grossPnl = calculateProjectedPnl({
        symbol,
        category,
        instrument,
        side,
    ...
    return {
        grossPnl,
        commission: commission == null ? fees.commission : Number.parseFloat(commission) || 0,
        swap: swap == null ? fees.swap : Number.parseFloat(swap) || 0,
        pnl: calculateNetPositionPnl({
            grossPnl,
            commission: commission == null ? fees.commission : Number.parseFloat(commission) || 0,
            swap: swap == null ? fees.swap : Number.parseFloat(swap) || 0,
        }),
