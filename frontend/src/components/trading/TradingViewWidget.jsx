...
const TradingViewWidget = memo(({
  ...
  return (
    <div style={{ height, width }} className={`flex flex-col h-full w-full rounded-[2rem] overflow-hidden shadow-2xl transition-all duration-300 ${isDark
        ? 'bg-slate-900 border border-slate-800'
        : 'bg-white border border-slate-200 shadow-slate-200/70'
      }`}>
      {/* Top Professional Toolbar */}
      <div className={`h-14 flex items-center justify-between px-6 border-b backdrop-blur-xl ${isDark
          ...
          <div className="flex items-center space-x-1">
            {intervals.map(t => (
              <button
                ...
                className={`px-3 py-1.5 text-[10px] font-black rounded-lg transition-all ${activeInterval === t
                    ? isDark
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex items-center space-x-3">
            <button className={`w-8 h-8 rounded-lg transition-colors flex items-center justify-center border ${isDark
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className={`w-12 border-r flex flex-col items-center py-4 space-y-5 ${isDark
            ? 'bg-slate-800/30 border-slate-700/50'
            : 'bg-slate-50 border-slate-200'
          }`}>
          {[FaSearch, FaChartBar, FaDrawPolygon, FaInfoCircle, FaCompress].map((Icon, i) => (
            <button key={i} className={`transition-colors p-2 rounded-xl ${isDark
        ...
        <div className={`flex-1 relative group ${isDark ? 'bg-slate-900/40' : 'bg-white'}`}>
          ...
          <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center space-x-2 backdrop-blur-md px-4 py-2 rounded-2xl border shadow-2xl z-20 opacity-0 group-hover:opacity-100 transition-opacity ${isDark
              ? 'bg-slate-800/80 border-slate-700'
              : 'bg-white/95 border-slate-200 shadow-slate-200/70'
            }`}>
            {['1D', '5D', '1M', '3M', '6M', 'YTD', '1Y', 'ALL'].map(r => (
              <button key={r} className={`text-[9px] font-black px-2 py-1 ${isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'
                }`}>{r}</button>
            ))}
          </div>
        </div>
        ...
        {/* Sidebar Watchlist */}
        <div className={`transition-all duration-500 ease-in-out border-l flex flex-col ${isDark ? 'border-slate-700/50' : 'border-slate-200'
          } ${isSidebarOpen ? 'w-72' : 'w-0 opacity-0 overflow-hidden'}`}>
          <div className={`p-5 flex items-center justify-between border-b ${isDark ? 'border-slate-700/50' : 'border-slate-200'
            }`}>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1 scrollbar-hide">
            {watchlist.map((asset) => (
              <div
                ...
                className={`flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-all duration-300 ${symbol === asset.symbol
              ...
              >
                <div>
                  ...
                  <p className={`text-[9px] font-medium leading-none ${symbol === asset.symbol
                      ? 'text-slate-900/60'
                      : isDark
                        ? 'text-slate-500'
                        : 'text-slate-400'
        ...
        {!isSidebarOpen && (
          <button
            ...
            className={`absolute right-0 top-1/2 -translate-y-1/2 p-1.5 border-l border-t border-b rounded-l-xl shadow-2xl z-30 ${isDark
      ...
      {/* Terminal Status Bar */}
      <div className={`h-8 border-t px-6 flex items-center justify-between ${isDark
