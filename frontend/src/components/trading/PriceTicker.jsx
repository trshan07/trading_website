      ...
      <div className="flex animate-ticker whitespace-nowrap py-3">
        {Object.entries(data).map(([symbol, info]) => (
          <div key={symbol} className="flex items-center mx-6">
            ...
            <span className="text-sm font-bold text-slate-900 dark:text-white transition-colors mr-2">${(info?.price ?? 0).toLocaleString()}</span>
            <span className={`text-xs font-bold px-1.5 py-0.5 rounded transition-colors ${(info?.change ?? 0) >= 0 ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10' : 'text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10'}`}>
