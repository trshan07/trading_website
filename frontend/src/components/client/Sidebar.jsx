...
const Sidebar = ({ activeTab, onTabChange, onLogout, user, portfolio, showBalance, onShowStatement = () => {} }) => {
  ...
  const metricValue = () => {
    if (activeMetric === 'balance')  return showBalance ? formatSidebarCurrency(portfolio?.totalBalance ?? 0) : 'â€¢â€¢â€¢â€¢â€¢â€¢';
    if (activeMetric === 'equity')   return showBalance ? formatSidebarCurrency(portfolio?.equity ?? 0) : 'â€¢â€¢â€¢â€¢â€¢â€¢';
    if (activeMetric === 'margin')   return showBalance ? formatSidebarCurrency(portfolio?.freeMargin ?? 0) : 'â€¢â€¢â€¢â€¢â€¢â€¢';
    if (activeMetric === 'pnl') {
      const pnl = portfolio?.dailyPnL ?? 0;
      return showBalance ? `${pnl >= 0 ? '+' : ''}${formatSidebarCurrency(pnl)}` : 'â€¢â€¢â€¢â€¢â€¢â€¢';
    }
  };
  ...
  const metricSub = () => {
    if (activeMetric === 'balance') return {
      ...
      value: showBalance ? formatSidebarCurrency(portfolio?.availableBalance ?? 0) : 'â€¢â€¢â€¢â€¢',
      color: (portfolio?.availableBalance ?? 0) >= 0 ? 'text-emerald-400' : 'text-rose-400',
    };
    if (activeMetric === 'equity') return {
      label: 'Unrealized PnL',
      value: showBalance ? `${(portfolio?.dailyPnL ?? 0) >= 0 ? '+' : ''}${formatSidebarCurrency(portfolio?.dailyPnL ?? 0)}` : 'â€¢â€¢â€¢â€¢',
  ...
  return (
    <aside className="w-72 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-r border-slate-200 dark:border-slate-800 hidden lg:flex flex-col h-screen sticky top-0 transition-all duration-300 z-50">

      {/* Header & Logo */}
      <div className="pt-5 pb-4 px-6 shrink-0 flex items-center border-b border-slate-100 dark:border-slate-800/80">
      ...
      <div className="px-4 pb-4 pt-4 shrink-0 transition-all duration-300">
      ...
      {/* Main Navigation */}
      <nav className="flex-1 space-y-0.5 min-h-0 overflow-y-auto custom-scrollbar px-4">
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full group py-3.5 flex items-center transition-all duration-300 relative px-4 rounded-[1.25rem] ${
                isActive
            ...
            >
              <div className={`p-2 rounded-xl border transition-all duration-300 flex-shrink-0 mr-3 ${
                isActive
        ...
        <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
          <button
            ...
            className="w-full group py-3.5 flex items-center transition-all duration-300 relative px-4 rounded-[1.25rem] text-slate-400 hover:text-gold-500 hover:bg-gold-50/50 dark:hover:bg-gold-500/10"
          >
            <div className="p-2 rounded-xl border transition-all duration-300 flex-shrink-0 mr-3 bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 group-hover:border-gold-300">
      ...
      {/* Footer / User Profile */}
      <div className="py-4 px-4 shrink-0 transition-all duration-300">
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 flex items-center justify-between p-3 group cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-all">
