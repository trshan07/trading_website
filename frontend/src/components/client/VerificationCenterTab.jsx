...
const UploadCard = ({ title, hint, file, setFile, existingDocument, onUpload }) => {
  const inputId = `kyc-${String(title).toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
  ...
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-5 sm:p-6 dark:border-slate-800 dark:bg-slate-900">
      ...
      <label
        htmlFor={inputId}
        className="mt-6 flex min-h-[170px] cursor-pointer flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 text-center transition-colors hover:border-slate-400 hover:bg-white dark:border-slate-700 dark:bg-slate-950/50 dark:hover:border-slate-600 dark:hover:bg-slate-950"
      >
        <input
          id={inputId}
          type="file"
